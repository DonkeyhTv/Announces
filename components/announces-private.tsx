...
const AnnouncesPrivate: React.FC = () => {
  ...
  useEffect(() => {
    const filtered = announces.filter((announce) => {
      ...
      if (!announce.status.includes("private")) return false;
      ...
      if (!matchesLocation && filters.iLiveIn.length > 0) {
        ...
        if (filterCity && userCity) {
          if (filters.distance.length === 0) {
            // If a city is selected but no distance is specified, only show announces from that city
            matchesLocation = user.address.city === filters.iLiveIn[0];
          } else {
    ...
    if (filters.iLiveIn.length > 0 && filters.distance.length > 0) {
...
export default AnnouncesPrivate;
